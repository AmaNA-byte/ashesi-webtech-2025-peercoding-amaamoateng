<?php
  ...
  // Get sessions with course information and attendance stats
...
<body>
            ...
            <tbody>
              <?php while ($session = mysqli_fetch_assoc($sessions_result)): ?>
                <tr>
                  ...
                  <td>
                    <span class="badge">
                      <?= $session['marked_students'] ?> / <?= $session['enrolled_students'] ?>
                    </span>
                  </td>
                  <td>
                    <div class="action-buttons">
                      <a href="generate_attendance_code.php?session_id=<?= $session['id'] ?>" 
                         class="btn" style="background-color: #15a541;">
                        Generate Code
                      </a>
                      <a href="mark_attendance.php?session_id=<?= $session['id'] ?>" 
                         class="btn outline">
                        Mark Attendance
                      </a>
                      <button onclick="editSession(<?= htmlspecialchars(json_encode($session)) ?>)" 
                              class="btn outline">
                        Edit
                      </button>
                      <form method="POST" style="display: inline;" 
                            onsubmit="return confirm('Are you sure you want to delete this session? All attendance records will also be deleted.')">
  ...
  <script>
    function editSession(session) {
      ...
      // Remove existing session_id if any
      const existingSessionId = form.querySelector('input[name="session_id"]');
      if (existingSessionId) {
        existingSessionId.remove();
      }
